...
fn make_native_structure(
    ...
    let tokens = quote! {
        ...
        use crate::classes::Node; // for user-facing object pointer methods
...
fn make_native_structure_field_and_accessor(
    ...
    if is_object_ptr {
        ...
        // Current native structures treat all object pointers as Node (only `collider`), so we can make this more type-safe.
        ...
        accessor = Some(quote! {
            ...
            pub fn #getter_name(&self) -> Option<Gd<Node>> {
            ...
            where T: crate::obj::Inherits<Node> {
